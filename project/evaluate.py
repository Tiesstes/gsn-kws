...
from project.data.dataset import SpeechCommandsKWS
...
def compute_metrics(all_predictions, all_labels, model_class_names):
    ...
    print(f"Precision (macro): {precision_score(all_labels, all_predictions, average='macro', zero_division=0):.4f}")
    print(f"Recall (macro):    {recall_score(all_labels, all_predictions, average='macro', zero_division=0):.4f}\n")

    for i, class_name in enumerate(model_class_names):

        print(f"{class_name:15s} | Precision: {precision[i]:.4f} | Recall: {recall[i]:.4f}")

    return confusion_matrix(all_labels, all_predictions), precision, recall
...
if __name__ == "__main__":
    ...
    dataset_indexer = torch.load(DATASPLIT_MANIFEST_PATH, map_location=DEVICE)
    finetune_split = dataset_indexer["indices"]["finetune"]  # Zachowujemy Twoją nazwę zmiennej
    ...
    # pełna mapa finetune
    test_dataset = SpeechCommandsKWS(dataset=base_data, split_indices=finetune_split["test"],
                                     speaker_id_map=dataset_indexer["maps"]["speaker_id_map_finetune"],
                                     noise_dir=NOISE_PATH, deterministic=True)
